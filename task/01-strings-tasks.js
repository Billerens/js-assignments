...
function extractNameFromTemplate(value) {
  return value.match(/Hello, (?<name>[\s\S]+?)!/).groups.name;
}
...
function encodeToRot13(str) {
  return str.replace(/[a-zA-Z]/g, function (match) {
    return String.fromCharCode(
      match.charCodeAt(0) + (match.toLowerCase() < "n" ? 13 : -13) // based on unicode
    );
  });
}
...
function getCardId(value) {
  var mapRank = { A: 0, J: 10, Q: 11, K: 12 };
  var mapSuit = { S9827: 0, S9830: 1, S9829: 2, S9824: 3 }; // map for card suits, keys according to unicode font code of each suit
  var id;
  id = !isNaN(parseInt(value)) // checking if it is not a named card (A, J, Q, K)
    ? mapSuit["S" + value.charCodeAt(value.length - 1)] * 13 + // if it is not - calculating by SuitCoefficient * 13 + CardWeight - 1 (bcs started from 0)
      (parseInt(value) - 1)
    : mapSuit["S" + value.charCodeAt(value.length - 1)] * 13 + // if it is named - SuitCoefficient * 13 + CardWeight (from map)
      mapRank[value[0]];
  return id;
}
